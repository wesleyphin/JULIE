...
class ReplaySession:
    ...
    def __post_init__(self) -> None:
        ...
        # NY-session filter: JULIE_REPLAY_NY_ONLY=1 restricts bar feed to 08:00–17:00 ET.
        # warmup/lookback stays full so ML features initialise correctly.
        if os.environ.get("JULIE_REPLAY_NY_ONLY", "0").strip() == "1":
            ny_idx = self.replay_df.index.tz_convert(NY_TZ)
            mask = (ny_idx.hour >= 8) & (ny_idx.hour < 17)
            self.replay_df = self.replay_df[mask]
            if self.replay_df.empty:
                raise ValueError("No replay bars remain after NY-session filter (08:00–17:00 ET).")
