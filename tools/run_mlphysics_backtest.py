...
def _resolve_run_dir(path_arg: str) -> Path:
    ...
    roots = [Path(path_arg).expanduser(), Path(__file__).resolve().parents[1] / Path(path_arg)]
    for root in roots:
        try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run MLPhysics-only backtests with all filters off.")
    ...
    parser.add_argument(
        "--dist-run-dir",
        default="",
        help="Override ML_PHYSICS_DIST_RUN_DIR for this backtest.",
    )
    parser.add_argument(
        "--disable-dist-cache",
        action="store_true",
        help="Disable MLPhysics dist prediction-cache reuse for this run.",
    )
    parser.add_argument("--report-dir", default="backtest_reports", help="Output directory for reports.")
    args = parser.parse_args()

    if str(args.dist_run_dir or "").strip():
        bt.CONFIG["ML_PHYSICS_DIST_RUN_DIR"] = str(_resolve_run_dir(str(args.dist_run_dir)))
    if bool(args.disable_dist_cache):
        opt_cfg = dict(bt.CONFIG.get("ML_PHYSICS_OPT", {}) or {})
        opt_cfg["enabled"] = True
        opt_cfg["mode"] = "backtest"
        opt_cfg["prediction_cache"] = False
        opt_cfg["overwrite_cache"] = False
        opt_cfg["dist_precomputed_file"] = ""
        bt.CONFIG["ML_PHYSICS_OPT"] = opt_cfg
        bt.CONFIG["ML_PHYSICS_DIST_LAZY_LOAD_BACKTEST"] = False
    ...
    print("selected_strategies=['MLPhysicsStrategy']")
    print("selected_filters=[]")
    print(f"dist_run_dir={bt.CONFIG.get('ML_PHYSICS_DIST_RUN_DIR')}")
    print(f"dist_cache_enabled={bool((bt.CONFIG.get('ML_PHYSICS_OPT', {}) or {}).get('prediction_cache', True))}")
