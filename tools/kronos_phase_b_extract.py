"""Kronos Phase B — extract Kronos forecast features for ALL DE3 + friend-rule
candidates (~1,599 rows). Saves resumable parquet artifact.

Run only if Phase A passes its decision criteria.

Outputs:
  artifacts/v18_kronos_features.parquet
"""
...
OUT_PARQUET = os.path.join(ROOT, "artifacts", "v18_kronos_features.parquet")
...
def kronos_features(pred_df, entry_price, inf_time_s):
...
def main():
    ...
    de3_fr["row_idx"] = de3_fr.index
    print(f"  DE3+friend candidates: {len(de3_fr)}")

    if args.limit > 0:
        de3_fr = de3_fr.iloc[:args.limit].copy()
    ...
    todo = de3_fr[~de3_fr["row_idx"].isin(done_idxs)].copy()
    print(f"  todo: {len(todo)}")

    symbols_needed = sorted(de3_fr["contract"].dropna().unique().tolist())
