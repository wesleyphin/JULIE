...
class OptimizedTPEngine:
    ...
    def check_sentiment_exit(self, position, current_time, es_price: float, kalshi) -> tuple[bool, str]:
        """
        Hour-turn exit gate using Kalshi crowd sentiment.
        """
        if kalshi is None:
            return False, ""
        ...
        if hasattr(position, "is_profitable"):
            profitable = bool(position.is_profitable(es_price))
        elif hasattr(position, "is_in_profit"):
            profitable = bool(position.is_in_profit)
        if not profitable:
            return False, ""
        ...
        if side == "LONG" and probability < 0.40:
            return True, f"HOUR-TURN EXIT: Crowd flipped bearish (prob={probability:.2f})"
        if side == "SHORT" and probability > 0.60:
            return True, f"HOUR-TURN EXIT: Crowd flipped bullish (prob={probability:.2f})"

        gradient = kalshi.get_probability_gradient(es_price)
        if gradient is None or abs(float(gradient)) >= 0.02:
            return False, ""

        pnl_pct = None
        if hasattr(position, "unrealized_pnl_pct"):
            try:
                pnl_pct = float(position.unrealized_pnl_pct(es_price))
            except Exception:
                pnl_pct = None
        if pnl_pct is not None and pnl_pct > 0.3:
            return True, f"HOUR-TURN EXIT: Crowd uncertain (gradient={gradient:.4f}), banking profit"
        return False, ""
