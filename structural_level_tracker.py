...
def _session_for_hour(hour: int) -> Optional[str]:
...
class StructuralLevelTracker:
    ...
    def update(
        ...
        """Feed one completed bar. `ts` should already be ET."""
        del c
        et_h = ts.hour
        et_m = ts.minute
        ...
        if session and not self._to_captured:
            ...
            if et_h == th and et_m == tm:
                ...
                logging.debug("StructuralLevelTracker: TrueOpen=%s %.2f", session, o)
    ...
    def get_active_levels(self) -> List[Dict]:
        ...
        def _add(price, name, ltype, priority) -> None:
    ...
    def get_state(self) -> dict:
        return {
            ...
            "_daily_open_date": (
                self._daily_open_date.isoformat() if self._daily_open_date else None
            ),
    ...
    def load_state(self, state: dict) -> None:
        ...
        daily_open_date = state.get("_daily_open_date")
        self._daily_open_date = (
            datetime.date.fromisoformat(daily_open_date) if daily_open_date else None
        )
