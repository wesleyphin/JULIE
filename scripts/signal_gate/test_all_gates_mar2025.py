"""Backtest all three v5 G gates + Kalshi on March 2025.

Strategies:
  DynamicEngine3  — from 2025_03/closed_trades.json (461 trades)
  RegimeAdaptive  — from 2025_03/closed_trades.json (66 trades)
  AetherFlow      — from af_fast_replay/mar2025/closed_trades.json

Per trade: Kalshi decision + G gate (v5, with regime-adaptive threshold).
"""
...
DE3_RA_SOURCE = ROOT / "backtest_reports" / "full_live_replay" / "2025_03" / "closed_trades.json"
AF_SOURCE = ROOT / "backtest_reports" / "af_fast_replay" / "mar2025" / "closed_trades.json"

# March 2025 weekly windows
WEEKS = [
    ("week1 (Mar 3-7)",  "2025-03-03", "2025-03-07"),
    ("week2 (Mar 10-14)","2025-03-10", "2025-03-14"),
    ("week3 (Mar 17-21)","2025-03-17", "2025-03-21"),
    ("week4 (Mar 24-28)","2025-03-24", "2025-03-28"),
    ("week5 (Mar 31)",   "2025-03-31", "2025-03-31"),
]
...
def compute_session_state(trades):
    """Per-trade: (cum_day_pnl_pre_trade, consec_losses_pre_trade, trades_today_pre_trade)."""
...
def main():
    ...
    master = master[(master.index >= "2025-02-15") & (master.index <= "2025-04-05")]
    print(f"  {len(master):,} rows (Mar 2025 + margin)")

    de3 = [t for t in load_trades(DE3_RA_SOURCE, "DynamicEngine3") if t.get("entry_time","").startswith("2025-03")]
    ra  = [t for t in load_trades(DE3_RA_SOURCE, "RegimeAdaptive") if t.get("entry_time","").startswith("2025-03")]
    af  = [t for t in load_trades(AF_SOURCE, "AetherFlow") if t.get("entry_time","").startswith("2025-03")]
    ...
    for strat in ["DynamicEngine3", "RegimeAdaptive", "AetherFlow"]:
        ...
        m = joblib.load(ROOT / f"artifacts/signal_gate_2025/model_{fam}.joblib")
        models[strat] = m
        print(f"  G[{strat}]: thr={m['veto_threshold']}, features={len(m['feature_names'])}, "
    ...
    for label, key in [("baseline","base"), ("Kalshi only","kal"),
                       ("G only (per-strat thr)","g"), ("Kalshi + G (FULL LIVE STACK)","full")]:
    ...
    print(f"\n\n{'='*96}\nFILTER EFFECT — delta vs baseline (March 2025)\n{'='*96}\n")
    print(f"  {'strategy':<18} {'baseline':>11} {'+Kalshi':>11} {'+G':>11} {'+Both':>11}  {'delta':>10}")
    print("  " + "-" * 85)
    total_base = total_full = 0
    for strat in all_trades:
        m = monthly[strat]
        db = m["base"]["pnl"]; dk = m["kal"]["pnl"]; dg = m["g"]["pnl"]; dfull = m["full"]["pnl"]
        total_base += db; total_full += dfull
        print(f"  {strat:<18} ${db:>+10.2f} ${dk:>+10.2f} ${dg:>+10.2f} ${dfull:>+10.2f}  ${dfull-db:>+9.2f}")
    print("  " + "-" * 85)
    print(f"  {'COMBINED':<18} ${total_base:>+10.2f} {'':>11} {'':>11} ${total_full:>+10.2f}  ${total_full-total_base:>+9.2f}")

    Path("/tmp/test_all_gates_mar2025.json").write_text(json.dumps({"monthly": monthly}, default=str, indent=2))
